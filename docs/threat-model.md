...
- AWS Budget alarm at $10/mo with a direct email subscriber — **built**
  (`BudgetAlarmConstruct`, `lib/budget-alarm.js`) and deployed with the stack.
...
- Eval framework exists as a local model-backed runner; CI runs the eval
  package unit tests, not the full model-backed corpus.
