 ...
 *
 * Design notes worth flagging for future slices:
 *
 ...
 *
 * - The Watches table partitions by userId, which is ideal for per-user CRUD
 *   from chat (slice 2). The poller (slice 5) needs every active watch
 *   across all users on each tick — that will require a Scan with a
 *   FilterExpression on status. Acceptable while watch counts are small
 *   (personal use, dozens of items). If this ever grows, add a GSI keyed
 *   on `status` so the poller can Query by status="active" instead.
 *
