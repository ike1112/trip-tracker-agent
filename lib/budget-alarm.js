...
class BudgetAlarmConstruct extends Construct {
    constructor(scope, id, props) {
        ...
        const email =
            scope.node.tryGetContext('budgetAlarmEmail') ??
            scope.node.tryGetContext('notifierRecipientEmail');
        ...
        if (!email || !EMAIL_PATTERN.test(email)) {
            throw new Error(
                `budgetAlarmEmail (or its notifierRecipientEmail fallback) is required ` +
                `and must look like an email; got: ${email}`
            );
        }
