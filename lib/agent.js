...
class AgentConstruct extends Construct {
    constructor(scope, id, props) {
        ...
        //                             servers, proving requests originate from this agent
        ...
        //
        // TODO(ADR 0006, ADR 0004): tighten this grant to the same
        // resource-scoped pattern `lib/poller-server.js` uses for the
        // poller's `bedrock:InvokeModel` grant. The agent's model ID is
        // resolvable at synth time via the same context pattern (or a
        // fixed allow-list); `Resource: *` here is the wider blast
        // radius in the same account that just tightened the poller.
        // Tracked alongside the per-component JWT-secret split.
