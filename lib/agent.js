...
class AgentConstruct extends Construct {
    constructor(scope, id, props) {
        ...
        // The IAM grant derives the foundation-model id by stripping a `us.`
        // prefix and enumerates the US destination Regions. A non-US
        // geographic profile (eu./apac.) would synthesise malformed
        // foundation-model ARNs and fail at runtime with AccessDenied.
        // Fail loud at synth instead — ADR 0006 documents that a non-US
        // profile needs US_PROFILE_REGIONS + this guard updated in code.
        if (!agentBedrockModelId.startsWith('us.')) {
            throw new Error(
                `agentBedrockModelId "${agentBedrockModelId}" is not a us. geographic ` +
                `inference profile; update US_PROFILE_REGIONS and this guard for a ` +
                `non-US profile (ADR 0006)`
            );
        }
